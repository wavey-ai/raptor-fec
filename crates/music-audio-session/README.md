...
sample-clock sequence and groups captured chunks into FEC micro-blocks. It
decodes incoming datagrams. Its playout buffer returns exact audio bytes or an
explicit missing-frame result.
...
Wavey's design uses TCP/TLS as the sustained-throughput and recording baseline.
Native fixed-deadline PCM uses paced UDP/RTP with immediate source shards. It
uses same-epoch XOR or small Reed-Solomon for repair. RaptorQ remains available
for large objects, backfill, and a measured high-channel same-epoch experiment.
